...
export const ResultQueue = Queue<ResultQueueParameter>(
  ...
  async (job, {id}) => {
    ...
    await fetch(url).then(async (result) => result.arrayBuffer())
      ...
      .then(async () => {
        await dbConnect()
        await Courses.bulkWrite<CoursesInterface>(
          bulkWrite.courses as unknown as AnyBulkWriteOperation<any>[]
        )
          .then((extra) => report.push({
            models: AllModelsEnum.Courses,
            status: ReportOptions.Created, extra
          }))
          .catch((extra) => report.push({
            models: AllModelsEnum.Courses,
            status: ReportOptions.Error, extra
          }))
        await Results.bulkWrite<ResultsInterface>(
          bulkWrite.results as unknown as AnyBulkWriteOperation<any>[]
        )
          .then((extra) => report.push({
            models: AllModelsEnum.Results,
            status: ReportOptions.Created, extra
          }))
          .catch((extra) => report.push({
            models: AllModelsEnum.Results,
        ...
        await Users.bulkWrite<UsersInterface>(
          bulkWrite.users as unknown as AnyBulkWriteOperation<any>[]
        )
          .then((extra) => report.push({
            models: AllModelsEnum.Users,
            status: ReportOptions.Created, extra
          }))
          .catch((extra) => report.push({
            models: AllModelsEnum.Users,
      ...
      .then(() => {
        let path = join(
          process.cwd(), 'public',
          'results-report', kebabCase(uploadedBy)
        )
        if (!existsSync(path)) {
          mkdirSync(path, {recursive: true})
        }
        path = join(path, id.concat('.json'))
